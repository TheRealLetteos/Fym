...
public class LevelConfig
{
    ...
    public static LevelConfig GetLevelConfig(int levelNumber)
    {
        ...
        int enemyCount = 5 + levelNumber / 2;
        float enemyLevel = 1 + levelNumber / 5;
        float spawnDensity = 0.05f + levelNumber / 100.0f;
        float spawnRange = 15.0f + levelNumber / 2.0f;
        float screenWidth = 1920.0f;
        float screenHeight = 1080.0f;
        float levelDifficulty = 1 + levelNumber / 3;
        return new LevelConfig(levelNumber, enemyCount, enemyLevel, spawnDensity, spawnRange, screenWidth, screenHeight, levelDifficulty);
    }
    ...
    public static LevelConfig GetNextLevelConfig()
    {
        ...
        CURRENT_LEVEL++;
        return GetLevelConfig(CURRENT_LEVEL);
    }
